...
const QuestionPage = () => {
  ...
  const handleNextQuestion = (type) => {
    if(isQuestionAnswered(questions?.question) && isAnswered){handleUpdateAnswer(questions.question, selectedOption.optionName)}
    else if(isAnswered){setUserAnswers(prevAnswers => [...prevAnswers, selectedOption])};
  ...
  const handleSubmit = () => {
      ...
      dispatch(storeUserAnswers(userAnswers, 0));
